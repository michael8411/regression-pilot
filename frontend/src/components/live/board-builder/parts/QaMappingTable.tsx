...
export function QaMappingTable({
  statuses,
  value,
  unresolved,
  onChange,
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <h3 className="text-[12.5px] font-semibold text-ink">QA mapping</h3>
        <button
          type="button"
          onClick={onAutoMap}
          disabled={statuses.length === 0}
          className="g-btn text-[11px] px-2 py-1 disabled:opacity-40"
        >
          Auto-map
        </button>
      </div>
      {statuses.length === 0 ? (
        <p className="text-[11.5px] text-ink-faint">
          Pick at least one status above to map it.
        </p>
      ) : (
        <ul className="flex flex-col gap-1.5">
          {statuses.map((s) => {
            const current = inferBucket(value, s);
            const isUnresolved = unresolved.includes(s);
            return (
              <li
                key={s}
                className="flex items-center justify-between gap-2 rounded-md border border-subtle bg-surface-elevated px-2.5 py-1.5"
              >
                <span className="text-[12px] text-ink truncate flex items-center gap-1">
                  {s}
                  {isUnresolved && (
                    <HelpCircle
                      size={11}
                      className="text-warn"
                      aria-label="Pick a bucket so this column has a home."
                    >
                      <title>
                        Pick a bucket so this column has a home.
                      </title>
                    </HelpCircle>
                  )}
                </span>
